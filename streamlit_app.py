        ...
        with st.container():
            ...
            with col1:
                if customer_text:
                    ...
                    st.write(
                        customer_text[:200] + "..." if len(customer_text) > 200 else customer_text
                    )
...
def main():
    ...
    st.markdown(
        "Predict customer churn using natural language descriptions powered by Large Language Models"
    )
    ...
    with st.sidebar:
        ...
        try:
            ...
            st.error(f"Error loading target mapping: {str(e)}")
    ...
    with tab1:
        ...
        st.markdown(
            "Enter a natural language description of a customer to predict their churn probability."
        )
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader(
            "Upload a text file with customer descriptions (one per line)", type=["txt"]
        )
        ...
        if batch_descriptions.strip():
            descriptions_to_process = [
                desc.strip() for desc in batch_descriptions.split("\n") if desc.strip()
            ]
        ...
        if descriptions_to_process:
            ...
            if st.button("🚀 Process Batch Predictions", type="primary"):
                with st.spinner(
                    f"Processing {len(descriptions_to_process)} customer descriptions..."
                ):
                    try:
                        all_results = []
                        all_extracted_data = []
                        ...
                        for idx, row in combined_results.iterrows():
                            ...
                            display_prediction_results(
                                pd.DataFrame([row]), row["customer_description"]
                            )
                        ...
                        download_data = combined_results[
                            [
                                "customer_index",
